...
export class RegionalPlanMappingPage {
  ...
  constructor(page: Page) {
    this.page = page;
    this.textToVerify = page.locator('t1-card-content > h4');
    this.buttonYes = page.locator('div.regional-plans > span');
    this.exchange = page.locator('//div[@class="base-plan-info"][contains(.,"TRINET III ")]');
    this.planName = (planName: string) =>
      page.locator(
        `//div[@row-index="0"]//span[translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')="${planName.toLowerCase()}"]`
      );
    this.planID = (planID: string) => page.getByRole('gridcell', { name: planID });
    ...
    this.closeIcon = page.locator('.cdk-overlay-backdrop');
  }
  ...
  async searchQuarter(quarter: string) {
    await this.utilities.executeStep(
      this.quarterSearchInput,
      'clear',
      'clearing search field to enter'
    );
    await this.utilities.executeStep(
      this.quarterSearchInput,
      'type',
      'search for quarter ' + quarter + '',
      quarter
    );
  }
  ...
  async closeSplitScreen() {
    await this.utilities.executeStep(
      this.closeIcon,
      'click',
      'clicking on close icon of split screen'
    );
  }
}
